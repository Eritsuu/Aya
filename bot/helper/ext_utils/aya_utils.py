    ...
    if name is not None:
        ...
        if telegraph_content:
            if config_dict['BOT_PM'] and message.chat.type != message.chat.type.PRIVATE:
                msg = f"File/Folder sudah tersedia di Drive.\n\nSaya telah mengirimkan tautan file yang tersedia di pm."
                pmmsg = f"Hai👋 {tag}.\n\nFile/Folder sudah tersedia di Drive.\n\nIni adalah {contents_no} Hasil yang Ditemukan:"
...
def checking_blacklist(message, button=None):
    ...
    if user_id in user_data and user_data[user_id].get('is_blacklist'):
        b_msg = f"<b>Kamu Di blacklisted ⚠️.</b>\n\n<b>User Id:</b> <code>{user_id}</code>.\n\n"
